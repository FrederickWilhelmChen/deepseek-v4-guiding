...
`|DSML|` 的意义可能不只是“换一种文本格式”，而是让模型在训练中反复见到一种稳定的工具调用边界标记。这样模型在生成工具调用时，可能更容易进入已经对齐过的格式轨道。

### 一些思考

这里需要明确区分事实和推测：

- **事实**：DS 在 V4 中使用了 `|DSML|` 和 XML 风格工具调用格式，并声称这种格式降低了工具调用错误。
- **推测**：在非官方 tools 参数场景下，手动使用 `|DSML|` 可能对 tool call 稳定性和指令跟随有帮助。

这个推测并不一定成立，因为 special token 的收益不只来自文本形式，也可能来自模型服务端、tokenizer、训练数据和推理框架的共同配合。实际使用中仍然需要实验验证。
...
事实上这也不是 DeepSeek 独有的问题。Kimi 和 Qwen 也有类似的 reasoning state 回传要求。现在很多开箱即用的 Agent 框架，例如 dify / LangChain / DeepAgents / CrewAI，往往都需要通过 provider / proxy 层面的特殊处理，才能兼容这类强依赖 `reasoning_content` 回传的模型。
...
> 高频 / 短输出 / 强依赖上下文的前置判断，不一定适合做成独立子 Agent；更合理的方向可能是让它变成主 Agent 共享上下文下的快速判定过程。
...
| 数学任务 | 长推理、严谨、可验证 |
| 代码任务 | 通过测试、能执行、能修复 |
| Agent 任务 | 多轮规划、工具调用、环境反馈闭环 |
| 普通对话 | 简洁、稳定、低延迟、指令跟随 |
| 写作 / 设计任务 | 难以定义唯一正确答案 |

进一步说，并不是所有任务都有明确正确答案。数学计算有对错，代码任务可以通过测试用例验证，但写作、页面设计、审美判断这类任务没有固定答案，也很难设计一个通用奖励目标。
...
这个思路对业务 Agent 也有启发：与其让一个主 Agent 同时学习所有领域行为，不如先做出多个强领域策略，再通过离线数据、轨迹回放、偏好标注或监督微调，把这些策略压回一个统一执行模型或统一 prompt policy 中。

但 OPD 对工程系统要求很高。因为它不是只需要一个老师，而是需要大量专家模型、rollout 数据、teacher scheduling、缓存和分布式训练框架配合。
...
如果说 5.1 回答的是“为什么要这样后训练”，那么 5.2 回答的就是：

> **这些后训练策略要如何在真实 GPU 集群、长上下文 rollout、海量教师模型和 Agent 沙箱中跑起来。**
...
这对业务 Agent 系统也有启发：如果线上真实运行环境和离线评测环境差异很大，离线优化出来的策略未必能在线上稳定复现。

例如：

- 离线环境工具很快，线上工具很慢；
- 离线环境上下文完整，线上会裁剪；
- 离线环境模型高精度，线上为了成本使用低精度；
- 离线环境沙箱稳定，线上沙箱有超时、资源抢占和文件系统差异。

后训练和评测都应该尽量贴近真实 runtime，否则优化目标会偏。
...
这部分思路与第三章 mHC 优化中的 recomputation 很相似：不把所有中间结果都存下来，而是在需要时重算其中一部分。
...
这套思路和 Agent 工程里的 tool / skill 管理非常像。

如果工具、技能、文档、样例全量塞进上下文，主 Agent 会被上下文压垮。更合理的做法是：
...
1. DS 的 GPU 集群设计了全集群维度的抢占式任务调度器。一个 agent 任务期间可能涉及大量工具调用，在模型发起工具调用和工具返回之间，GPU 事实上不工作，形成空闲。调度器会利用这类空闲派发其他任务，因此任何 running task 都可能随时被抢占资源。
...
这个设计最有意思的地方不是“能恢复”，而是 DS 清楚解释了为什么不能简单从 0 重新生成。
...
这说明 DS 的后训练已经不是传统“prompt-answer 数据训练”，而是强依赖真实执行环境。

尤其对 coding / agentic task 来说，模型输出不是静态文本，而是要进入环境执行、观察结果、修复错误、继续行动。没有沙箱，就无法可靠采集这类真实轨迹，也无法做大规模 RL / OPD。

### 一些思考

这对业务 Agent 平台也很关键。一个真正能自我改进或长期优化的 Agent 系统，不能只存聊天记录，还需要存：
...
1. 公开 benchmark 可能存在训练数据污染风险，也可能被模型厂商针对性优化。因此 benchmark 更适合观察趋势，而不适合直接等同于真实能力排序。
...
RAG 更像一次或少数几次检索，然后把检索结果塞进上下文。Agentic Search 则更像一个可以多轮搜索、改写查询、交叉验证、跟随线索继续探索的搜索过程。

复杂任务里，用户问题往往不是一次检索就能解决的。模型需要根据中间发现动态改写搜索方向，这正是 agentic search 的优势。
...
- 简单事实查询，RAG 足够；
- 复杂研究任务，agentic search 更强；
- 需要交叉验证、补充检索、动态分解的问题，单次 RAG 很容易不够。

但 agentic search 也不是免费午餐。它会引入更多搜索轮次、更复杂的上下文管理，以及更高的不确定性。真正的系统设计应该根据任务复杂度动态选择，而不是用一种搜索模式覆盖所有场景。
...
尤其是 PPT 视觉设计，它不是纯文本问题，而是需要空间布局、审美、信息密度控制和工具调用共同配合。单靠更长 reasoning，不一定能解决视觉设计问题。

对于 Agent 产品来说，这也意味着：如果任务本身需要视觉布局或高压缩摘要，单纯提高模型推理档位未必是最优解。更实际的做法可能是引入专门的结构化模板、视觉评估器、排版约束和后处理工具。
...
第五章真正值得关注的，不是 benchmark 分数，而是它展示了一个趋势：

> **后训练正在从“给模型更多偏好数据”变成“围绕真实 Agent runtime 构建完整训练系统”。**

这一章里最重要的几个信号是：

1. reasoning effort 被显式分档，推理预算成为可调资源；
2. tool call schema、special token 和 interleaved thinking 开始深度绑定；
3. thinking state 正在成为 Agent runtime 的一等状态；
4. Quick Instruction 说明高频上下文相关判断不一定适合拆成小模型；
5. OPD 和 GRM 分别解决多目标能力整合和 hard-to-verify 任务评价；
6. FP4 rollout、teacher scheduling、WAL、million-token data loader 和 DSec 说明后训练已经高度依赖真实工程系统；
7. benchmark 更适合观察趋势，不适合直接当作模型能力排序。

用一句话概括：

> **DeepSeek V4 的 Post-Training 不是单纯强化学习章节，而是在说明大模型如何围绕长上下文、工具调用、真实环境和可恢复轨迹，进入更完整的 Agent 时代。**
