...
| General Infrastructure | 工程基础设施 | Kernel、训练框架、KV Cache | 高 |
| Pre-Training | 预训练工程 | 数据、训练节奏、稳定性、基模评测 | 中 |
| Post-Training | 后训练工程 | GRM、OPD、FP4 QAT、reasoning / agent / coding 对齐 | 中 |
...
- **FP4 QAT**：后训练阶段引入低精度部署形态，使 rollout 与线上部署更一致。
...
> 需要强调一点，deepseek实际上开源了两套精度，一套有Base后缀，专家权重使用FP8。另一套的专家权重使用FP4。本仓库中放置的两个config.json均是后者的配置
...
> **本文将 DeepSeek V4 报告概括为一个核心问题：如何同时支持超长上下文、长程推理和可承受成本。**
...
- 由于本文考虑主要面对非算法背景读者，因此对算法的数学过程做了大量简化，很多细节都没有展开，敬请算法背景读者谅解
- 为了可读性、通俗性和流畅性，本文在很多算法和技术细节上做了不小的简化和概括，也没有完全忠实于原文章节结构，敬请谅解。如需深入细节，建议直接阅读也已放在仓库中的原文报告 pdf
- 本文为了让 coder 们直观感受到一些核心数学过程，在 `/scripts` 路径下放置了三个演示具体算法的 python 脚本。这些脚本仅负责演示算法的核心数学过程，仅仅是 toy case，并非算法本身。
