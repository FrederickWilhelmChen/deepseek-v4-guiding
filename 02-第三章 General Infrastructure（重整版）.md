...
这一节本质上是在回答一个很朴素的问题：MoE 理论上省算力，为什么工程上却不一定省时间。DeepSeek 给出的答案不是继续压缩通信，而是把通信重新编排进计算时序里。

---

# 🛠️ 3.2 TileLang：DeepSeek 自己的 kernel 平台

这一节讨论的是另一个基础设施问题：当标准算子已经不够用时，如何在“全部手写 CUDA”和“完全依赖框架默认实现”之间找到中间层。
...
为什么这个这么重要？因为 kernel 运算里包含大量索引计算，会带来越界、转换等价性、地址冲突等问题，必须要一个求解器来做整数分析，判断优化是不是安全的。如果不安全，就只能保守处理；反之就可以做更激进的优化。

## 💡 这一节的核心

TileLang 的价值不只是“写出更快的 kernel”，而是让 DeepSeek 有能力把越来越多非标准结构稳定地沉淀成可复用的工程资产。
...
大前提是：<text underline="true">*计算机系统的浮点数运算中，实数的加法结合律并不成立*</text>。
...
运行上面这段 Python 代码，`a` 的值是 `1`，而 `b` 的值是 `0`。这就是 <text underline="true">*浮点数加法不满足结合律*</text>。也就是说，在浮点数运算里，即使是同一批数，顺序位置不同，最后的结果也可能完全不同。
...
这一节的重点不只是“让结果更稳定”，而是把“可复现性”当成一种一等工程能力。没有 determinism，很多训练问题甚至没有进入可调试阶段。
...
这一节真正想表达的是：低精度不是部署阶段的补丁，而应该成为训练阶段就被系统性建模的目标。
...
这一节说明的是：长上下文模型的推理系统，已经不能把 KV cache 看成一种单一形态的数据结构。只要注意力机制开始混合压缩和稀疏选择，缓存系统就必须跟着一起重写。
